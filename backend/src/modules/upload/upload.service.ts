...
export class UploadService {
  ...
  /**
   * 上传头像（本地存储到 uploads/images/avatar/ 目录）
   */
  uploadAvatar(file: any, req?: any) {
    const filePath = `/uploads/images/avatar/${file.filename}`;
  ...
  /**
   * 上传头像到腾讯云COS（avatar目录）
   */
  async uploadAvatarToCos(file: any, req?: any): Promise<any> {
    // 修复文件名编码
    const originalname = this.fixFilenameEncoding(file.originalname);
    ...
    // COS对象键（文件路径）- 保存到 avatar 目录
    const key = `avatar/${uniqueName}`;
    ...
    try {
      ...
      throw new InternalServerErrorException(`头像上传失败: ${error.message || '未知错误'}`);
    }
  }
